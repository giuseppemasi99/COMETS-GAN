...
class MyLightningModule(pl.LightningModule):
    ...
    def validation_epoch_end(self, samples: Sequence[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]) -> None:
        ...
        pred_volumes = pred_volumes.squeeze().cpu()
        sequence = sequence.squeeze().cpu()
        prices = prices.squeeze().cpu()
        volumes = volumes.squeeze().cpu()

        # pred_sequence.shape = [8, 4290]
        # pred_prices.shape = [4, 4290]
        # pred_volumes.shape = [4, 4290]
        # prices.shape = [4, 4290]
        # volumes.shape = [4, 4290]

        self.log_multistock_prices(sequence[:4], pred_sequence[:4], pred_prices, prices)
        self.log_multistock_volumes(sequence[4:], pred_sequence[4:], pred_volumes, volumes)

        self.log_metrics_volume(volumes.numpy(), pred_volumes.numpy())
        self.log_multistock_minmax_volumes(volumes.numpy(), pred_volumes.numpy())
    ...
    def predict_autoregressively(
        ...
        return torch.Tensor(pred_sequence), torch.Tensor(pred_prices), torch.Tensor(pred_volumes)
    ...
    def log_multistock_prices(
        self, sequence: torch.Tensor, pred_sequence: torch.Tensor, pred_prices: torch.Tensor, prices: torch.Tensor
    ) -> None:

        # sequence.shape = [4, 4290]
        # pred_sequence.shape = [4, 4290]
        # prices.shape = [4, 4290]
        # pred_prices.shape = [4, 4290]
    ...
    def log_multistock_volumes(
        self, sequence: torch.Tensor, pred_sequence: torch.Tensor, pred_volumes: torch.Tensor, volumes: torch.Tensor
    ) -> None:
